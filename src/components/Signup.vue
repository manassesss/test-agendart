<template>
  <Transition name="fade-slide" mode="out-in">
    <div class="relative bg-white/40 border border-white p-8 sm:p-10 text-center flex flex-col justify-between items-center shadow-lg rounded-lg w-full max-w-sm min-h-[550px]">
      ...
      <p class="text-md font-semibold text-[#1E1E1E] text-center">Crie sua conta para continuar.</p>

      <form @submit.prevent="signup" class="w-full flex flex-col gap-y-4">
        <div class="flex flex-col text-left w-full">
          <label class="block font-medium text-[#1e1e1e]">Nome</label>
          <input v-model="name" type="text" class="w-full h-12 bg-white px-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>
      ...
      <p class="text-center text-gray-600 text-sm mt-4">
        Já tem conta? <button @click="$emit('switch')" class="text-blue-500 underline">Entrar</button>
      </p>
